...
<div class="card">
    ...
    <form action="{{route('admins.admin.store')}}" method="post" class="p-5">
        @csrf
            <div class="row d-flex justify-content-lg-end">
        <h3 class="">Thêm quản trị viên</h3>
            </div>
                @if ($errors->any())
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="email" name="email" value="{{old('email')}}"   placeholder="@example.com" >
                 @error('email')
                <span style="color: red" class="text-sm">{{$message}}</span>
            @enderror
            </div>
