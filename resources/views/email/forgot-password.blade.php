          ...
          <div class="row d-flex justify-content-center">
            <div class="col-xl-6 col-lg-5 col-md-7 d-flex flex-column mx-lg-0 mx-auto">
              <div class="card card-plain">
                ...
                <div class="card-body">
                  {{-- form login --}}
                  <form role="form" action="{{ route('postReset', ['token'=>$token]) }}" method="POST" >
                    @csrf
                    @include('message')
                    <div class="mb-3">
                      <input type="password" class="form-control form-control-lg" placeholder="Password" name="password" value="{{old('password')}}">
                    </div>
                    <input type="password" class="form-control form-control-lg" placeholder="Confirm password" name="confirm" value="{{old('confirm')}}">
                    </div>
                    <div class="text-center">
                      <button type="submit" class="btn btn-lg btn-primary btn-lg w-50 mt-4 mb-0">Thay đổi </button>
                    </div>
                  </form>
                  {{-- end form login --}}
                </div>
